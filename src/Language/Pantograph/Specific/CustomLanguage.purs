...
instance EncodeJson DataType where
  encodeJson a = genericEncodeJson a

instance DecodeJson DataType where
...
instance Eq InfixOperator where
  eq x = genericEq x

instance Ord InfixOperator where
  compare x y = genericCompare x y
...
instance EncodeJson InfixOperator where
  encodeJson a = genericEncodeJson a

instance DecodeJson InfixOperator where
  decodeJson a = genericDecodeJson a
...
instance Grammar.IsRuleLabel PreSortLabel RuleLabel where
  ...
  defaultDerivTerm' (MInj (Grammar.SInj TermSort) % [ gamma, ty ]) = pure (Grammar.makeLabel TermHole [ "gamma" /\ gamma, "type" /\ ty ] % [ sortToType ty ])
...
language = TotalMap.makeTotalMap case _ of
  ...
  InfixRule op ->
    let
      { left, right, output } = infixTypes op
    in
...
arrangeDerivTermSubs _ { renCtx: preRenCtx, rule, sort, sigma, dzipper, mb_parent, renderTerm } =
  let
    renCtx = preRenCtx { cssClasses = Set.delete "error" preRenCtx.cssClasses } :: Base.RenderingContext
  in -- remove some things from css classes
    let
      parentRule = case mb_parent of
        Just (Tooth (DerivLabel rule _) _) -> Just rule
        Nothing -> Nothing
    in
      let
        dotOrNot = case mb_parent of -- Used in Var and App cases
          Just (Tooth (DerivLabel label _) (ZipList.Path { left, right })) | RevList.length left == 0 -> isAppOrGreyApp label
          _ -> false
      in
        let
          html =
            ( case rule /\ sort of
                _ /\
                  ...
                  let
                    classes = if locality == sor Local % [] then [] else [ "error", "free" ]
                  in
                ...
                Lam /\ _ ->
                  let
                    maybeMapsTo = case dzipper of
                      Just (Zipper _ (_ % [ _, _, (DerivLabel Lam _) % _ ])) -> [ Rendering.spaceElem ]
                      _ -> [ mapstoElem ]
                  in
                    let
                      renCtx' = if parentRule == Just Lam then renCtx else Base.incremementIndentationLevel renCtx
                    in
                      [ pure [ lambdaElem ]
                      , Left (renCtx /\ 0)
                      , pure [ colonElem ]
                ...
                Let /\ _ ->
                  let
                    renCtx' = Base.incremementIndentationLevel renCtx
                  in
                    [ pure [ letElem ]
                    , Left (renCtx' /\ 0)
                    , pure [ colonElem ]
                ...
                label /\ _ | isAppOrGreyApp label -> -- App and GreyApp
                  let
                    isGrey = case label of
                      GreyApp -> true
                      _ -> false
                  in
                    let
                      leftParen /\ rightParen = case mb_parent of -- Used in Var and App cases
                        Just (Tooth (DerivLabel label _) (ZipList.Path { left, right: _ }))
                          | RevList.length left == 0
                          , isAppOrGreyApp label -> [] /\ []
                        _ -> [ Rendering.lparenElem ] /\ [ Rendering.rparenElem ]
                    in
                      let
                        renCtx' = Base.incremementIndentationLevel renCtx
                      in
                        let
                          renCtx'' = if isGrey then renCtx' { cssClasses = Set.insert "grey" (Set.singleton "error") } else renCtx'
                        in
                ...
                ArrowRule /\ _ ->
                  let
                    leftParen /\ rightParen = case mb_parent of -- Used in Var and App cases
                      Just (Tooth (DerivLabel ArrowRule _) (ZipList.Path { left, right: _ })) | RevList.length left == 0 -> [ Rendering.lparenElem ] /\ [ Rendering.rparenElem ]
                      _ -> [] /\ []
                  in
                    let
                      renCtx' = Base.incremementIndentationLevel renCtx
                ...
                If /\ _ ->
                  let
                    renCtx' = Base.incremementIndentationLevel renCtx
                  in
                    [ pure [ ifElem, Rendering.spaceElem ]
                    , Left (renCtx' /\ 0)
                    , pure ((newlineIndentElem renCtx.indentationLevel) <> [ thenElem, Rendering.spaceElem ])
                    , Left (renCtx' /\ 1)
                    , pure ((newlineIndentElem renCtx.indentationLevel) <> [ elseElem, Rendering.spaceElem ])
                    , Left (renCtx' /\ 2)
                    ]
                {-
            ErrorBoundary -> Grammar.makeRule ["gamma", "insideType", "outsideType"] \[gamma, insideType, outsideType] ->
          ...
          -}
                ...
                ListMatchRule /\ _ ->
                  let
                    renCtx' = Base.incremementIndentationLevel renCtx
                  in
                    [ pure [ HH.b_ [ HH.text "match " ] ]
                    , Left (renCtx' /\ 0)
                    , pure [ HH.b_ [ HH.text " with" ] ]
                    , pure (newlineIndentElem renCtx.indentationLevel)
                    , pure [ HH.text "nil => " ]
                    , Left (renCtx' /\ 1)
                    , pure (newlineIndentElem renCtx.indentationLevel)
                    , pure [ HH.text "cons " ]
...
getFreeVar ctx fvName fvType = matchExpr2 ctx
  (sor CtxConsSort %$ [ slot, slot, slot ])
  ( \[ name, ty, ctx' ] ->
...
maximallyApplied cursorCtx cursorTy tyOft toBeWrapped =
  ...
  in
    case matchExprImpl tyOft (sor Arrow %$ [ slot, slot ]) of
      Just [ a, b ] ->
        let
          wrapped = Grammar.makeLabel App [ "gamma" /\ cursorCtx, "a" /\ a, "b" /\ b ]
            % [ toBeWrapped, Grammar.makeLabel TermHole [ "gamma" /\ cursorCtx, "type" /\ a ] % [ sortToType a ] ]
        in
...
getVarEdits sort =
  matchExpr2 sort (sor TermSort %$ [ slot, slot ])
    ( \[ cursorCtx, cursorTy ] ->
        let
          indices = getIndices cursorCtx
        in
          let
            makeEdit index =
...
getWrapInAppEdit name cursorSort dterm =
  matchExpr2 cursorSort (sor TermSort %$ [ slot, slot ])
    ( \[ cursorCtx, cursorTy ] ->
...
getVarWraps cursorSort
  | Just [ cursorCtx, _cursorTy ] <- matchExprImpl cursorSort (sor TermSort %$ [ slot, slot ]) =
      let
        meta = fromMetaVar (freshMetaVar "any")
      in
        ...
        in
          ...
          in
            List.concat (List.mapMaybe (\x -> x) edits)
getVarWraps _ = Nil
...
splitChange c =
  case ChangeAlgebra.eliminateReplaces c of
    c | Maybe.Just ([] /\ [ ctx, ty ]) <- matchChange c (TermSort %+- [ cSlot, cSlot ]) ->
      let
        _ctx1 /\ ctx2 = ChangeAlgebra.endpoints ctx
      in
        let
          ty1 /\ _ty2 = ChangeAlgebra.endpoints ty
        in
    ...
    c | Maybe.Just ([] /\ [ _ty ]) <- matchChange c (TypeSort %+- [ cSlot ]) -> { upChange: c, cursorSort: lEndpoint c, downChange: ChangeAlgebra.inject (lEndpoint c) }
...
editsAtCursor cursorSort =
  Array.mapMaybe identity
    ...
    <>
      if not (isTermSort cursorSort) then []
      else
        Array.mapMaybe identity
...
insertSucRule = Smallstep.makeDownRule
  ...
  ( \[ y, typeY ] [ ctx, x, typeX, locality ] [ i ] ->
      pure $
        dTERM Suc [ "gamma" /\ rEndpoint ctx, "x" /\ rEndpoint x, "typeX" /\ rEndpoint typeX, "y" /\ y, "typeY" /\ typeY, "locality" /\ rEndpoint locality ]
...
localBecomesNonlocal = Smallstep.makeDownRule
  ...
  (Zero %# [])
  ( \[ a, ty ] [ ctx, a', ty' ] [] ->
      if not (ChangeAlgebra.inject a == a' && ChangeAlgebra.inject ty == ty') then Maybe.Nothing
      else
...
nonlocalBecomesLocal = Smallstep.makeDownRule
  ...
  ( \[ a, ty ] [ ctx, a', ty' ] [ _i ] ->
      ...
      if not (ChangeAlgebra.inject a == a' && Maybe.isJust (unify ty (rEndpoint ty'))) then Maybe.Nothing
      else
        pure $ Smallstep.wrapBoundary Smallstep.Up
          ( csor VarSort %
              [ (csor CtxConsSort % [ a', ChangeAlgebra.inject ty, ChangeAlgebra.inject (rEndpoint ctx) ])
              , a'
...
passThroughArrow _ dterm =
  case dterm of
    ( (Smallstep.Boundary Smallstep.Down ch) %
      ...
      | Just ([] /\ [ ca, cb ]) <- matchChange ch (TypeSort %+- [ Arrow %+- [ {-ca-} cSlot, {-cb-} cSlot ] ]) -> pure
          ( dTERM ArrowRule [ "a" /\ rEndpoint ca, "b" /\ rEndpoint cb ]
...
wrapLambda = Smallstep.makeDownRule
  ...
  ( \[ a ] [ gamma, b ] [ t ] ->
      let
        varName = (MInj (Grammar.DataLabel (DataString "")) % [])
      in
        pure $
...
unWrapLambda
  _
  ( Expr (Smallstep.Boundary Smallstep.Down ch)
      [ Expr (SSInj (Grammar.DerivLabel Lam sigma)) [ _name, _ty, body ]
      ]
  ) = do
  let varName = Util.lookup' (RuleMetaVar "x") sigma
  restOfCh <- case unit of
    _ | Just ([ a ] /\ [ gamma, b ]) <- matchChange ch (TermSort %+- [ {-gamma-} cSlot, dMINUS Arrow [ {-a-} slot ] {-b-}  cSlot [] ]) -> pure (csor TermSort % [ minusChange (sor CtxConsSort) [ varName, a ] gamma [], b ])
    _ -> Nothing
  pure
...
upWrapLambdaUp
  _
  ( Expr (SSInj (Grammar.DerivLabel Lam sigma))
  ...
  =
      pure $
        let
          _ = trace "NOTE: this rule is untested, because I couldn't find a case where it was used." \_ -> unit
        in
          wrapBoundary Up (csor TermSort % [ gamma, (csor Arrow) % [ inject a, b ] ])
            $ wrapBoundary Down
                ( csor TermSort %
                    [ minusChange (sor CtxConsSort)
                        [ rEndpoint x, a ]
                        (inject (rEndpoint gamma))
                        []
                    , (csor Arrow) % [ inject a, inject (rEndpoint b) ]
                    ]
                )
                t
...
wrapApp th ssterm =
  Smallstep.makeUpRule
    ...
    ( \[ t ] -> t /\
        ( \[ a ] [ gamma, b ] inside ->
            if not (isNeutralFormWithoutCursor inside) then Nothing
            else
              pure
...
makeAppGreyed
  _
  ( (SSInj (Grammar.DerivLabel App _)) %
      [ (Smallstep.Boundary Smallstep.Up ch) % [ inside ]
      , arg
      ]
  )
  | Just ([ a ] /\ [ gamma, b ]) <- Expr.matchChange ch (TermSort %+- [ {-gamma-} cSlot, dMINUS Arrow [ {-a-} slot ] {-b-}  cSlot [] ]) = do
...
mergeAppGreyApp _ dterm = case dterm of
  ( (SSInj (DerivLabel GreyApp greySigma)) %
    ...
    | Util.lookup' (RuleMetaVar "a") sigma == Util.lookup' (RuleMetaVar "a") greySigma -> pure (SSInj (DerivLabel App sigma) % [ t1, t2 ])
  _ -> Nothing
...
removeError _ (Expr.Expr (SSInj (Grammar.DerivLabel ErrorBoundary sigma)) [ t ]) =
  let
    insideType = Util.lookup' (Expr.RuleMetaVar "insideType") sigma
  in
    let
      outsideType = Util.lookup' (Expr.RuleMetaVar "outsideType") sigma
    in
...
removeErrorPermissive _ (Expr.Expr (SSInj (Grammar.DerivLabel ErrorBoundary sigma)) [ t ]) =
  ...
  in
    ...
    in
      let
        gamma = Util.lookup' (Expr.RuleMetaVar "gamma") sigma
      in
        trace "GONNA DO IT:" \_ ->
...
mergeErrors
  _
  ( Expr.Expr (SSInj (Grammar.DerivLabel ErrorBoundary sigma1))
      [ Expr.Expr (SSInj (Grammar.DerivLabel ErrorBoundary sigma2)) [ t ]
      ]
  ) =
  let
    insideInside = Util.lookup' (Expr.RuleMetaVar "insideType") sigma2
  in
    let
      outsideOutside = Util.lookup' (Expr.RuleMetaVar "outsideType") sigma1
    in
      let
        gamma = Util.lookup' (Expr.RuleMetaVar "gamma") sigma1
      in
...
mergeErrorsUp
  _
  ( Expr.Expr (SSInj (Grammar.DerivLabel ErrorBoundary sigma))
      [ Expr.Expr (Boundary Up ch) [ t ]
      ]
  )
  | Just ([] /\ [ gamma, c ]) <- matchChange ch (TermSort %+- [ {-gamma-} cSlot, {-c-} cSlot ]) =
      let
        insideInside = rEndpoint c
      in
        let
          outsideOutside = Util.lookup' (Expr.RuleMetaVar "outsideType") sigma
        in
          pure
            $ wrapBoundary Up (csor TermSort % [ gamma, inject outsideOutside ])
            $
              dTERM ErrorBoundary [ "gamma" /\ rEndpoint gamma, "insideType" /\ insideInside, "outsideType" /\ outsideOutside ] [ t ]
mergeErrorsUp _ _ = Nothing

mergeErrorsDown :: StepRule
mergeErrorsDown
  _
  ( Expr.Expr (Boundary Down ch)
      [ Expr.Expr (SSInj (DerivLabel ErrorBoundary sigma)) [ t ]
      ]
  )
  | Just ([] /\ [ gamma, c ]) <- matchChange ch (TermSort %+- [ {-gamma-} cSlot, {-c-} cSlot ]) =
      let
        insideInside = Util.lookup' (RuleMetaVar "insideType") sigma
      in
        let
          outsideOutside = rEndpoint c
        in
          pure $ dTERM ErrorBoundary [ "gamma" /\ rEndpoint gamma, "insideType" /\ insideInside, "outsideType" /\ outsideOutside ]
...
fallbackDownError _ (Expr.Expr (Boundary Down ch) [ inside@(SSInj _ % _) ]) -- Only do the down rule if there is a regular derivterm below, if we did this on markers and Boundaries it might create a problem
  ...
  | Just ([] /\ [ gamma, c ]) <- matchChange ch (TermSort %+- [ {-gamma-} cSlot, {-c-} cSlot ])
  , Just [ _gamma, insideTy ] <- matchExprImpl (Smallstep.ssTermSort inside) (sor TermSort %$ [ {-gamma-} slot, {-insideTy-} slot ]) = trace "fallbackDownError called" \_ ->
    ...
    -> do
      (gamma /\ c /\ inside /\ insideTy) /\ i <- flip Util.findWithIndex kids \_index -> case _ of
        (Boundary Up ch % [ inside ])
          | Just ([] /\ [ gamma, c ]) <- matchChange ch (TermSort %+- [ {-gamma-} cSlot, {-c-} cSlot ])
          , Just [ _gamma, insideTy ] <- matchExprImpl (Smallstep.ssTermSort inside) (sor TermSort %$ [ {-gamma-} slot, {-insideTy-} slot ]) -> pure $ gamma /\ c /\ inside /\ insideTy
        _ -> Nothing
      traceM ("fallbackUpError called")
      let outsideTy = lEndpoint c
      pure $
        ( SSInj label % Util.fromJust
            ( Array.updateAt i
                ( wrapBoundary Up (csor TermSort % [ gamma, inject outsideTy ]) $
                    dTERM ErrorBoundary [ "gamma" /\ rEndpoint gamma, "insideType" /\ insideTy, "outsideType" /\ outsideTy ] [ inside ]
                )
                kids
            )
        )
...
introDownErrorNeutral parentTooth (Expr.Expr (Boundary Down ch) [ inside@(SSInj _ % _) ]) -- Only do the down rule if there is a regular derivterm below, if we did this on markers and Boundaries it might create a problem
  ...
  , case parentTooth of
      ...
      _ -> true
  , Just [ _gamma, insideTy ] <- matchExprImpl (Smallstep.ssTermSort inside) (sor TermSort %$ [ {-gamma-} slot, {-insideTy-} slot ]) = pure $ dTERM ErrorBoundary [ "gamma" /\ rEndpoint gamma, "insideType" /\ insideTy, "outsideType" /\ rEndpoint c ]
      [ wrapBoundary Down (csor TermSort % [ gamma, inject insideTy ]) inside ]
...
onDelete cursorSort
  | Maybe.Just [ ty ] <- matchExprImpl cursorSort (sor TypeSort %$ [ slot ]) = csor TypeSort % [ replaceChange ty (fromMetaVar (freshMetaVar "deleted")) ]
...
generalizeDerivation sort
  | Maybe.Just [ ctx, ty ] <- matchExprImpl sort (sor TermSort %$ [ slot, slot ]) = csor TermSort % [ ChangeAlgebra.diff ctx startCtx, ChangeAlgebra.inject ty ]
...
specializeDerivation clipboard cursor
  ...
  , Maybe.Just [ cursorCtx, _cursorTy ] <- matchExprImpl cursor (sor TermSort %$ [ slot, slot ]) = csor TermSort % [ ChangeAlgebra.diff clipCtx cursorCtx, ChangeAlgebra.inject clipTy ]
...
clipboardSort s
  | Maybe.Just [ gamma, ty ] <- matchExprImpl s (sor TermSort %$ [ slot, slot ]) = sor TermSort % [ startCtx, fromMetaVar (freshMetaVar "anyType") ]
...
extraQueryEdits cursorSort query
  | Just [ cursorCtx, cursorTy ] <- matchExprImpl cursorSort (sor TermSort %$ [ slot, slot ]) =
      case Int.fromString query of
        ...
        Just n ->
          let
            dterm = makeLabel IntegerLiteral [ "gamma" /\ cursorCtx, "n" /\ MInj (DataLabel (DataInt n)) % [] ]
